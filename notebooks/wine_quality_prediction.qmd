...
The Wine Quality dataset comes relatively clean with no missing values. We combine the features and target into a single DataFrame and save the processed data for downstream analysis. The dataset has `{python} wine_df_shape[0]` rows and `{python} wine_df_shape[1]` columns.

@tbl-variables contains the physicochemical properties and sensory quality ratings of red and white wines found within the dataset. Each row represents a single wine sample, with 11 continuous features describing chemical characteristics such as fixed acidity, volatile acidity, citric acid, residual sugar, chlorides, sulfur dioxide, density, pH level, sulphates, and alchohol levels. The target variable, `quality`, is an ordinal score from `{python} min_quality`-`{python} max_quality` assigned by human tasters based on sensory evaluation.
...
quality_mode =  int(wine_df["quality"].mode()[0])

Markdown(wine_df_describe_table.to_markdown(index=False))
```
...
Most wines have moderate acidity, low volatile acidity, and around 9-11% alcohol, with a median quality of `{python} quality_mode`.
...
median_quality = round(float(wine_df['quality'].median()), 2)
...
@fig-wine-quality-distribution-bar is a distrubution of wine quality scores as a bar chart, showing how many wines in the dataset fall into each quality category.

The target variable, Wine Quality, is approximately normally distributed, with a mean of `{python} mean_quality` and a mode of `{python} mode_quality`. Most wines fall in the mid-quality range of `{python} mean_quality-std_quality` - `{python} mean_quality+std_quality`, indicating that extremely low or high quality wines are less common.
