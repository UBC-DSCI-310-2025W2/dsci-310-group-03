...
To model the relationship between the chemical inputs and the quality output, we employ a $K$-Nearest Neighbors (KNN) regression approach [@hastie_tibshirani_friedman_2009]. KNN is a natural starting point for this prediction task because it makes no assumptions about the underlying relationship between chemical features and quality — it simply predicts based on the ratings of the most chemically similar wines. This is well-suited to our dataset, where feature–quality relationships may be non-linear and complex. While more sophisticated models (e.g., Random Forests or Gradient Boosting) could capture higher-order interactions, KNN provides an interpretable baseline that directly leverages the intuition that wines with similar chemical profiles should taste similar. By standardizing the chemical features and using cross-validation to select the optimal $K$, our model attempts to predict a wine's quality score from the ratings of its nearest chemical neighbors.
